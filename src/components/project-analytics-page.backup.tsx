...
function profileFromMember(member: { id: string; name: string; accumulatedPoints: number; creditScore: number }): RadarProfile {
  const seed = seedFromName(member.name);
  const base = Math.max(45, Math.min(98, member.accumulatedPoints + member.creditScore - 5));
  const dimensions = Array.from({ length: 6 }).map((_, index) => {
    const variance = ((seed + index * 17) % 23) - 11;
...
function RadarChart({ values, average }: { values: number[]; average: number[] }) {
  ...
  const axis = axisPoints(values.length, 92);
  ...
  return (
    <svg viewBox="0 0 240 240" className="h-52 w-52">
      {rings.map((ring) => (
        <polygon
          key={ring}
          points={points(Array(values.length).fill(ring), 92)}
          fill="none"
          stroke="#d9dee8"
          strokeDasharray="3 5"
          strokeWidth="1"
      ...
      {axis.map((line, index) => (
        <line key={index} x1="120" y1="120" x2={line.x} y2={line.y} stroke="#e5e7eb" strokeWidth="1" />
      ))}
      <polygon
        points={points(average, 92)}
        fill="none"
        stroke="#9ca3af"
        strokeDasharray="4 4"
        strokeWidth="1.5"
      />
      <polygon points={points(values, 92)} fill="rgba(30, 64, 175, 0.15)" stroke="#1d4ed8" strokeWidth="2" />
      {axis.map((item, index) => (
        <text key={index} x={item.x} y={item.y} fill="#6b7280" fontSize="10" textAnchor="middle" dominantBaseline="middle">
          {METRIC_LABELS[index]}
        </text>
      ))}
    </svg>
...
export function ProjectAnalyticsPage({ projectId }: { projectId: string }) {
  ...
  const profiles = data.members.map(profileFromMember).sort((a, b) => b.totalScore - a.totalScore);
      ...
      <div className="shell py-6">
        ...
        {activeProfile ? (
          <section className="mt-6 rounded-2xl bg-white p-4 text-sm text-muted shadow-[0_10px_28px_rgba(15,23,42,0.06)]">
            ...
            综合分 <span className="font-medium text-slate-900">{activeProfile.totalScore}</span>，
            可优先关注其低于团队均值的维度进行辅导。
          </section>
        ) : null}
      </div>
